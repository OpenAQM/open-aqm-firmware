...
class OaqmUI {
  public:
    class OaqmUIProgress {
      public:
        OaqmUIProgress(short x, short y, short width);
        virtual ~OaqmUIProgress();
    ...
    class OaqmUINotification {
      public:
        OaqmUINotification(std::string message, bool isPersistent);
    ...
    OaqmUI();
    static OaqmUI* getInstance();
    static void resetInstance();

    void loop();
    void setRootApplication(OaqmAppV2* rootApplication);
    OaqmAppV2* getRootApplication();
  ...
  private:
    static std::unique_ptr<OaqmUI> instance;
    unsigned long mTargetFPS = 30;
    String mProgressText;
    OaqmUIProgress* mProgressBar = nullptr;
    unsigned int lastFlush = 0;
    unsigned int lastBGFlush = 0;
    std::mutex mNotificationsLock;
    std::list<OaqmUINotification> mNotifications;
    OaqmAppV2* mRootApplication = nullptr;
};
