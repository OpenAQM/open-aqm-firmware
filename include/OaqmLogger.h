...
class OaqmLogger {
  public:
    ...
    static inline OaqmLogger* getInstance() {
        if (instance == nullptr)
            instance.reset(new OaqmLogger());
        return instance.get();
    };
  ...
  private:
    static std::unique_ptr<OaqmLogger> instance;
    std::mutex m_lock;
...
#ifndef NDEBUG
#define OAQM_LOG_D(message...) OaqmLogger::getInstance()->debug(__FILE__, __LINE__, message)
#else
#define OAQM_LOG_D(...)
#endif
#define OAQM_LOG_I(message...) OaqmLogger::getInstance()->info(__FILE__, __LINE__, message)
#define OAQM_LOG_W(message...) OaqmLogger::getInstance()->warning(__FILE__, __LINE__, message)
#define OAQM_LOG_E(message...) OaqmLogger::getInstance()->error(__FILE__, __LINE__, message)